...
def main():
    ...
    for i, seed in enumerate(seeds):
        ...
        agent = QLearningAgent(
            gamma=config["gamma"], learning_rate=config["learning_rate"], epsilon=config["epsilon"]
        )
        ...
        avg_score, scores = agent.evaluate(
            board_size=config["board_size"], num_episodes=num_eval_games
        )
        ...
        print(
            f"  Avg: {avg_score:.2f} | Best: {best_score} | Worst: {worst_score} | Fail rate: {fail_rate:.1f}%"
        )
