...
class ExperimentLogger:
    ...
    def __init__(
        ...
        if use_tensorboard and TENSORBOARD_AVAILABLE:
            ...
            print(
                "Warning: TensorBoard requested but not installed. Install with: pip install tensorboard"
            )
...
if __name__ == "__main__":
    ...
    logger = ExperimentLogger(
        experiment_name="demo", config={"learning_rate": 0.001, "gamma": 0.99}
    )

    for step in range(10):
        logger.log_metrics(
            {"score": np.random.randint(0, 100), "loss": np.random.random()}, step=step
        )
