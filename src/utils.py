...
def plot_training_curves(
    ...
    ax4.axvline(np.mean(final_scores), color='r', linestyle='--', linewidth=2, label=f'Mean: {np.mean(final_scores):.1f}')
...
def print_training_summary(training_results: dict, coverage: dict) -> None:
    ...
    print(f"  Fully Explored: {coverage['fully_explored_states']} ({coverage['full_exploration_percentage']:.1f}%)")
